...
pub trait CacheStrategy: Send + Sync {
    ...
    fn get(
        &self,
        key: &str,
    ) -> impl std::future::Future<Output = CacheResult<Option<Vec<u8>>>> + Send;
...
impl CacheAsideStrategy {
    ...
    pub async fn get_or_fetch<F, T>(&self, key: &str, fetch_fn: F, ttl: Duration) -> CacheResult<T>
    where
        ...
        match self.cache.get::<T>(key).await {
            Ok(Some(value)) => {
                REDIS_OPERATIONS_TOTAL
                    .with_label_values(&["cache_aside_hit"])
                    .inc();
            ...
            Ok(None) => {
                REDIS_OPERATIONS_TOTAL
                    .with_label_values(&["cache_aside_miss"])
                    .inc();
            ...
            Err(e) => {
                ...
                REDIS_OPERATIONS_TOTAL
                    .with_label_values(&["cache_aside_error"])
                    .inc();
...
impl WriteThroughStrategy {
        ...
        T: Serialize,
    {
        ...
        self.cache.set(key, value, Some(ttl)).await?;
        REDIS_OPERATIONS_TOTAL
            .with_label_values(&["write_through_set"])
            .inc();
    ...
    pub async fn get<T: for<'de> Deserialize<'de>>(&self, key: &str) -> CacheResult<Option<T>> {
        ...
        if result.is_ok() {
            REDIS_OPERATIONS_TOTAL
                .with_label_values(&["write_through_get"])
                .inc();
        }
        result
    }
    ...
    pub async fn invalidate(&self, key: &str) -> CacheResult<()> {
        ...
        REDIS_OPERATIONS_TOTAL
            .with_label_values(&["write_through_invalidate"])
            .inc();
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_write_through_semantics() {
        ...
        assert!(
            db_updated && cache_updated,
            "Both DB and cache must be updated"
        );
    }
