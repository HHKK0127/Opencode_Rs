...
mod day13_api_caching_tests {
    ...
    #[tokio::test]
    async fn test_01_file_metadata_cache() {
        // Test 1: File metadata caching
        // Validates that file metadata can be cached with proper key format
    ...
    #[tokio::test]
    async fn test_02_file_list_cache() {
        // Test 2: File list caching with pagination
        // Validates separate cache entries for different pagination parameters
        ...
        // Cache different pagination combinations
        let page1_data = b"page 1 data".to_vec();
        let page2_data = b"page 2 data".to_vec();
        let page1_50_data = b"page 1 50 items".to_vec();

        cache.set(&list_key_page1, page1_data.clone()).await;
        cache.set(&list_key_page2, page2_data.clone()).await;
        cache.set(&list_key_different_size, page1_50_data.clone()).await;

        // Verify separate cache entries
        assert_eq!(cache.get(&list_key_page1).await.unwrap(), page1_data);
        assert_eq!(cache.get(&list_key_page2).await.unwrap(), page2_data);
        assert_eq!(
            cache.get(&list_key_different_size).await.unwrap(),
            page1_50_data
        );
    ...
    #[tokio::test]
    async fn test_03_search_results_cache() {
        // Test 3: Search results caching with query hashing
        // Validates search queries are hashed for cache keys
        ...
        // Generate cache keys and cache results
        for (query, result) in &queries {
            let query_hash = format!("{:x}", fxhash::hash64(query));
            let cache_key = format!("files:search:{}", query_hash);
            cache.set(&cache_key, result.to_vec()).await;
        }
    ...
    #[tokio::test]
    async fn test_04_cache_hit_rate() {
        // Test 4: Cache hit rate calculation
        // Simulates realistic access patterns to measure hit rate
        ...
        for key in &keys {
            cache.set(key, b"cached data".to_vec()).await;
        }

        // Simulate access pattern: 10 reads with 7 hits, 3 misses
        for _ in 0..7 {
            let _ = cache.get("file:1").await;
        }

        for _ in 0..3 {
            let _ = cache.get("file:nonexistent").await;
        }
    ...
    #[tokio::test]
    async fn test_05_cache_invalidation() {
        // Test 5: Cache invalidation patterns
        // Validates pattern-based cache invalidation

        let cache = MockCache::new();

        // Setup: Cache multiple entries with different patterns
        let entries = vec![
            ("file:metadata:123", b"metadata"),
            ("file:metadata:456", b"metadata"),
            ("files:list:1:20", b"list"),
            ("files:list:2:20", b"list"),
            ("files:search:abc123", b"search"),
            ("files:stats", b"stats"),
        ];
        ...
        // Simulate invalidation: pattern-based deletion
        ...
        // Verify: metadata entries deleted, others remain
        assert!(cache.get("file:metadata:123").await.is_none());
        assert!(cache.get("file:metadata:456").await.is_none());
        assert!(cache.get("files:list:1:20").await.is_some());
        assert!(cache.get("files:search:abc123").await.is_some());
    ...
    #[tokio::test]
    async fn test_06_large_dataset_caching() {
        // Test 6: Caching large file lists
        // Validates cache handles large datasets efficiently
        ...
        cache.set(cache_key, large_list.clone()).await;

        // Verify large data is cached
        let cached = cache.get(cache_key).await;
        assert!(cached.is_some());
        assert_eq!(cached.unwrap().len(), large_list.len());

        // Test multiple accesses to large dataset
        for _ in 0..10 {
    ...
    #[tokio::test]
    async fn test_07_cache_memory_efficiency() {
        // Test 7: Cache memory usage and efficiency
        // Validates memory usage with multiple cache entries

        let cache = MockCache::new();

        // Calculate memory for 100 file metadata entries
        ...
        for i in 0..100 {
            ...
            cache
                .set(&format!("file:metadata:{}", i), bytes)
                .await;
        }
        ...
        assert_eq!(
            storage.len(),
            100,
            "Should have 100 cached entries"
        );

        // Estimate memory: each entry ~80 bytes + key ~20 bytes
...
// Helper: Simple hash function (would use real crypto in production)
mod fxhash {
