...
fn test_01_cache_effect_measurement() {
    ...
    assert!(
        ...
        "Cache hit rate should be > 95%, got {:.1}%",
...
fn test_05_concurrent_access_performance() {
    ...
    // 10,000 total operations, mostly hits
    assert_eq!(metrics.operations, 10000);
    assert!(
...
fn test_08_performance_improvement_baseline() {
    ...
    let improvement = baseline_latency as f64 / hit_latency as f64;

    assert!(
        improvement > 2.0,
        "Cache should provide at least 2x improvement, got {:.1}x",
        improvement
    );
